  ...
  #+begin_src sh
    if command -v tv >/dev/null 2>&1; then
       ...
       function tv_shell_history() {
           ...
           local output
           output=$(tv bash-history --input "$current_prompt")
...
** Cable Channels
   :PROPERTIES:
   :header-args: :mkdirp yes :tangle ~/.config/television/cable_channels.toml
   :END:

   Modified from https://github.com/alexpasmantier/television/blob/main/cable/unix-channels.toml
   ...
   #+begin_src toml
     ...
     [[cable_channel]]
     name = "bash-history"
     source_command = "sed '1!G;h;$!d' ${HISTFILE:-${HOME}/.bash_history} | rg -v '^#' | nl | sort -k2 -u | sort -nr | cut -f 2"
   #+end_src
