...
* OpenRouter
  ...
  The actual secret =OPENROUTER_API_KEY= — grab one at https://openrouter.ai/keys — lives with my other [[*Secrets][secrets]] in =~/.bashrc.d/secret/variables.sh=.
  This section only /documents/ it, so the block below is illustrative and intentionally /not/ tangled; nothing here is
  ...
  configured as part of [[https://eca.dev/][eca]]'s =openrouter= provider — see the =eca= section above — since eca is the thing
  consuming this key.
...
** openusage
   ...
   only the OpenRouter API key, so like the variables above it is treated as secret and is /not/ tangled or reproduced
   here.
