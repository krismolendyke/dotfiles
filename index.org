...
* OpenTofu
   :PROPERTIES:
   :header-args: :mkdirp yes :tangle ~/.tofurc
   :END:
...
* OpenRouter
  ...
  I route the [[https://docs.claude.com/en/docs/claude-code/overview][Claude Code]] CLI through [[https://openrouter.ai][OpenRouter]] rather than talking to Anthropic directly.  Claude Code
  honors a handful of =ANTHROPIC_*= environment variables, so pointing =ANTHROPIC_BASE_URL= at OpenRouter and
  authenticating with an OpenRouter key is enough to redirect it.

  The actual =export=s live with my other [[*Secrets][secrets]] in =~/.bashrc.d/secret/variables.sh= (that is where the secret
  =OPENROUTER_API_KEY= — grab one at https://openrouter.ai/keys — is kept and where these variables are really set).
  This section only /documents/ them, so the block below is illustrative and intentionally /not/ tangled; nothing here
  is exported a second time at runtime.
...
** openusage
   ...
   [[https://www.openusage.ai/][openusage]] tracks my OpenRouter spend.  Its configuration lives at =~/.config/openusage/openrouter.json= and holds
   only the OpenRouter API key, so like the routing variables above it is treated as secret and is /not/ tangled or
   reproduced here.
