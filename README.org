...
*** =PATH=
    ...
    #+BEGIN_SRC sh
      ...
      if [[ -x /usr/local/go/bin/go ]]; then
          ...
          pathmunge "$(/usr/local/go/bin/go env GOPATH)/bin"
      fi
    #+END_SRC
...
**** =kubectl=, =k=, =kctx=, =kns=
     ...
     #+BEGIN_SRC sh
       if snap services microk8s.daemon-apiserver | tr -s ' ' | tail -1 | cut -d ' ' -f 2 | grep --quiet ^active; then
           alias k='microk8s.kubectl'
           source <(microk8s.kubectl completion $(basename ${SHELL}) | sed 's/kubectl/k/g')
       fi
     #+END_SRC
     ...
     #+BEGIN_SRC sh
       [ -e "$(pkg-config --variable=completionsdir bash-completion)/kubens" ] && source "$(pkg-config --variable=completionsdir bash-completion)/kubens" && alias kns="kubens"
     #+END_SRC
