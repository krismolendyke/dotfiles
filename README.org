...
* [[https://github.com/lotabout/skim][skim]]
  ...
  #+begin_src sh
    export SKIM_DEFAULT_OPTIONS="--ansi --bind 'alt-a:select-all+accept,ctrl-o:execute(emacsclient --no-wait {})+accept' --prompt '❯ ' --cmd-prompt 'C❯ ' --color "$(if k20e_macos_is_dark > /dev/null 2>&1; then echo 'dark'; else echo 'light'; fi)" --inline-info --multi"
  #+end_src
