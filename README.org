...
*** Python
    ...
    #+BEGIN_SRC sh
      function k20e/pip-upgrade() {
          ...
          pip install --upgrade --requirement ${CODE_HOME}/requirements-to-freeze.txt
          pip freeze > ${CODE_HOME}/requirements.txt
      }
    #+END_SRC
...
* Python
  :PROPERTIES:
  :header-args: :tangle ~/Code/requirements-to-freeze.txt
  :END:
