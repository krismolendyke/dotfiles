...
* Build Configuration Files
  :PROPERTIES:
  :CUSTOM_ID: build-configuration-files
  :END:
...
** Change Default Shell
   :PROPERTIES:
   :CUSTOM_ID: change-default-shell
   :END:
...
** Configuration
   ...
   #+begin_src sh
     ...
     alias ....="cd ../../.."
     alias dirs="dirs -v"
     function k20e_cd_root() {
         local root
         root="$(jj root 2>/dev/null || git rev-parse --show-toplevel 2>/dev/null)" && [ -n "$root" ] && [ "${PWD}" != "$root" ] && { cd "$root" || return; }
     }
     alias r=k20e_cd_root
     alias tree="tree -C"
...
* Secrets
  ...
  Setup environment and source secrets:

  #+begin_src sh
    export K20E_SECRET_ENV="${K20E_SECRET_HOME}/secrets.env"
    touch -a "${K20E_SECRET_ENV}"

    if [ -f "${K20E_SECRET_ENV}" ]; then
        set -a
        # shellcheck source=/dev/null
        source "${K20E_SECRET_ENV}"
        set +a
    fi
  #+end_src
  ...
  #+begin_src sh
    ...
    [ -f "${K20E_SECRET_ENV}" ] && chmod 0600 "${K20E_SECRET_ENV}"
  #+end_src
...
* Carapace
  ...
  #+begin_src sh
    if command -v carapace >/dev/null 2>&1; then
        ...
        # Bridge common aliases to their underlying carapace completer
        _carapace_alias_completer() {
            ...
            if [[ -n "$target" ]]; then
                COMP_WORDS[0]="$target"
                local prefix="${COMP_LINE:0:${#alias_name}}"
                if [[ "$prefix" == "$alias_name" ]]; then
                    COMP_LINE="${target}${COMP_LINE:${#alias_name}}"
                    ((COMP_POINT += ${#target} - ${#alias_name}))
                fi
            fi
            _carapace_completer "$@"
        }
        complete -o default -o bashdefault -F _carapace_alias_completer g j k c 2>/dev/null || true
    fi
  #+end_src
...
* Claude
  ...
  To route Claude Code through OpenRouter on personal machines without polluting the ambient environment or
  interfering with enterprise credentials on work machines, use OpenRouter's official [[#ori][ori]] harness: =ori claude=.

* [[https://difftastic.wilfred.me.uk/][Difftastic]]
  :PROPERTIES:
  :CUSTOM_ID: difftastic
  :header-args: :mkdirp yes :tangle ~/.bashrc.d/112-difftastic.bash
  :END:

  =difft= has no configuration file; its defaults are set through =DFT_*= environment variables (see =difft --help=).
  Explicit flags win over these, which lets callers that want something else override them, e.g. [[#git][Git]] passes
  =--display=inline=.

  Setting the display mode here, rather than in [[#jujutsu][Jujutsu]]'s =ui.diff-formatter= argv, lets =jj= use its built-in
  =merge-tools.difft= invocation unmodified.  That invocation passes =--width=$width=, which =difft= needs because its
  output is piped to a pager and it therefore cannot detect the terminal width on its own (it falls back to 80 columns).

  #+begin_src sh
    export DFT_DISPLAY=side-by-side-show-both
  #+end_src

  The [[#config-nu][Nushell]] equivalent lives in =config.nu=.
...
* [[https://git-scm.com/][Git]]
  ...
  #+begin_src sh
    alias g="git"
    type __git_complete >/dev/null 2>&1 && __git_complete g __git_main || true
  #+end_src
  ...
  #+begin_src sh
    [ -e "${HOMEBREW_PREFIX}/opt/git/share/git-core/contrib/diff-highlight/" ] && pathmunge "${HOMEBREW_PREFIX}/opt/git/share/git-core/contrib/diff-highlight/"
  #+end_src
...
* =kubectl=, =k=, =kctx=, =kns=, =krew=
  :PROPERTIES:
  :CUSTOM_ID: kubectl-k-kctx-kns-krew
...
* Nushell
  ...
  The complete Nushell configuration is grouped here for easy review. Follow the links to each tool's main section for shared documentation and Bash configuration.
...
** =config.nu=
   ...
   #+begin_src nu
     ...
     $env.PATH = (
         ...
         | prepend ($env.HOME | path join 'code/go/bin')
         | prepend ($env.HOME | path join '.local/bin')
         | prepend ($env.HOME | path join 'bin')
         | prepend '/opt/homebrew/Caskroom/google-cloud-sdk/latest/google-cloud-sdk/bin'
         | uniq
     )
     ...
     # Load secrets from ~/.bashrc.d/secret/secrets.env or an optional specified file
     def --env k20e_load_secrets [file?: path] {
         let secrets_env = ($file | default ($env.HOME | path join ".bashrc.d/secret/secrets.env"))
         if ($secrets_env | path exists) {
             open $secrets_env
             | lines
             | where { $in =~ '^[A-Za-z0-9_]+=' }
             | parse "{key}={value}"
             | reduce -f {} {|it, acc| $acc | insert $it.key ($it.value | str trim -c '"' | str trim -c "'") }
             | load-env
         }
     }

     # Initial load of secrets on shell startup
     k20e_load_secrets
     ...
     # Quick runner for bash one-liners / scripts
     def --wrapped b [...cmd] {
         ^bash -c ($cmd | str join ' ')
     }
     ...
     def --env r [] {
         ...
         let root = if $jj_root.exit_code == 0 {
             $jj_root.stdout | str trim
         } else {
         ...
         if ($root | is-not-empty) and ($root != $env.PWD) {
             cd $root
         }
     }
     ...
     let carapace_cache = ($nu.data-dir | path join "vendor/autoload/carapace.nu")
     if not ($carapace_cache | path exists) and (is-installed carapace) {
         mkdir ($nu.data-dir | path join "vendor/autoload")
         carapace _carapace nushell | save -f $carapace_cache
     }

     let carapace_aliases = {
         g: "git"
         j: "jj"
         k: "kubectl"
         c: "claude"
     ...
     def carapace-completer [spans: list<string>] {
         let expanded_spans = if ($spans.0 in $carapace_aliases) {
             $spans | skip 1 | prepend ($carapace_aliases | get $spans.0)
         } else {
             $spans
         }

         with-env { CARAPACE_LENIENT: "1" } {
             ^carapace $expanded_spans.0 nushell ...$expanded_spans
         } | from json
     }
     ...
     if not ($starship_cache | path exists) and (is-installed starship) {
         mkdir ($nu.data-dir | path join "vendor/autoload")
...
** Emacs
   :PROPERTIES:
   :CUSTOM_ID: emacs-nushell
   :header-args: :mkdirp yes :tangle ~/Library/Application Support/nushell/autoload/107-emacs.nu
   :END:

   See [[#emacs][Emacs]].

   #+begin_src nu
     # Support opening files at a line number with the :linum suffix, mirroring k20e_ec
     def ec [file?: string, ...rest] {
         if ($file != null) and ($file =~ '^(.+):(\d+)(?::\d+)?$') {
             let m = ($file | parse -r '^(?P<path>.+):(?P<line>\d+)')
             ^emacsclient -n $"+($m.0.line)" $m.0.path ...$rest
         } else if ($file != null) {
             ^emacsclient -n $file ...$rest
         } else {
             ^emacsclient -n ...$rest
         }
     }
...
** Skills Wrapper
   :PROPERTIES:
   :CUSTOM_ID: skills-nushell-wrapper
   :header-args: :mkdirp yes :tangle ~/Library/Application Support/nushell/autoload/109-skills.nu
   :END:

   See [[#skills][Skills]].

   #+begin_src nu
     def k20e_agent_skills_sync [...args] {
         ^k20e-skills-sync ...$args
     }
   #+end_src

** eza
   :PROPERTIES:
   :CUSTOM_ID: eza-nushell
   :header-args: :mkdirp yes :tangle ~/Library/Application Support/nushell/autoload/130-eza.nu
   :END:
...
** Television
   :PROPERTIES:
   :CUSTOM_ID: television-nushell
   :header-args: :mkdirp yes :tangle ~/Library/Application Support/nushell/autoload/140-television.nu
   :END:

   See [[#television][Television]].
   ...
   #+begin_src nu
     ...
     if (which tv | is-not-empty) {
         $env.config.keybindings = (
             $env.config.keybindings
             | append [
                 # tv_smart_autocomplete on Ctrl-T omitted so Ctrl-T transposes characters
                 {
...
*** History Channel
    :PROPERTIES:
    :CUSTOM_ID: nushell-history
    :header-args: :mkdirp yes :tangle ~/.config/television/cable/nu-history.toml
    :END:
...
** [[https://openrouter.ai/ori/harness][ori]]
   :PROPERTIES:
   :CUSTOM_ID: ori
   :END:

   [[https://openrouter.ai/ori/harness][ori]] is OpenRouter's official harness for running coding agent CLIs (Claude Code, Codex, Hermes, OpenCode, Pi, etc.)
   on OpenRouter models with OAuth PKCE authentication.  Because =ori= authenticates via browser OAuth and wraps agent
   processes directly, it avoids the need to export global =ANTHROPIC_*= environment variables, ensuring enterprise
   credentials remain completely untouched on work machines.

   Install or update:

   #+begin_src sh :tangle no
     # Install standalone ori to ~/.local/bin/ori (on PATH for Bash and Nushell)
   ...
   Launch Claude Code via OpenRouter:

   #+begin_src sh :tangle no
     ori claude
   #+end_src

** Environment & Downstream Tools
   :PROPERTIES:
   :CUSTOM_ID: openrouter-env
   :END:

   For non-harness tools that consume OpenRouter directly (such as [[#eca][eca]], [[#openusage][OpenUsage]], LiteLLM, or local routers like
   [[https://github.com/prismhq/jev-router][jev-router]]), the secret =OPENROUTER_API_KEY= from https://openrouter.ai/keys lives with my other [[*Secrets][secrets]] in
   =~/.bashrc.d/secret/secrets.env= (managed via personal 1Password vault).
   ...
   The actual OpenRouter provider/model routing config (which models, privacy/data-collection preferences, etc.) for eca is
   configured as part of [[https://eca.dev/][eca]]'s =openrouter= provider — see the =eca= section above.
...
* Skills
  ...
  :CUSTOM_ID: skills
   ...
   Synchronizes external agent skills (e.g. =krismolendyke/jujutsu-skill=) to
...
** Standalone CLI (=~/.local/bin/k20e-skills-sync=)
   :PROPERTIES:
   :CUSTOM_ID: skills-sync-cli
   :header-args: :mkdirp yes :tangle ~/.local/bin/k20e-skills-sync :shebang "#!/usr/bin/env bash"
   :END:
   ...
   #+begin_src bash
     ...
     K20E_AGENT_SKILL_REPOS=(
         "https://github.com/krismolendyke/jujutsu-skill"
     )

     cache_dir="${XDG_DATA_HOME:-$HOME/.local/share}/agent-skills"
     mkdir -p "$cache_dir"
     for url in "${K20E_AGENT_SKILL_REPOS[@]}"; do
         ...
         if [[ ! -d "$clone_dir/.jj" && ! -d "$clone_dir/.git" ]]; then
             jj git clone "$url" "$clone_dir" >/dev/null 2>&1
         else
             jj -R "$clone_dir" git remote set-url origin "$url" 2>/dev/null || true
             jj -R "$clone_dir" git fetch --remote origin --quiet
             jj -R "$clone_dir" new 'trunk()' >/dev/null 2>&1
         fi
         trunk_info="$(jj -R "$clone_dir" log -r 'trunk()' --no-graph -T 'change_id.shortest() ++ ": " ++ description.first_line()' 2>/dev/null || true)"
         while IFS= read -r skill_file; do
             skill_dir="$(dirname "$skill_file")"
             skill_name="$(basename "$skill_dir")"
             for agent_dir in \
                 "$HOME/.claude/skills/$skill_name" \
                 "$HOME/.config/eca/skills/$skill_name" \
                 "$HOME/.gemini/skills/$skill_name" \
                 "$HOME/.gemini/config/skills/$skill_name"; do
                 mkdir -p "$agent_dir"
                 cp -R "$skill_dir/"* "$agent_dir/"
             done
             echo "✓ Synced $skill_name from $url"
             if [[ -n "$trunk_info" ]]; then
                 echo "  Trunk tip: $trunk_info"
             fi
...
** =~/.config/starship.toml=
    :PROPERTIES:
   :CUSTOM_ID: config-starship-toml
...
*** https://starship.rs/config/#go
    :PROPERTIES:
    :CUSTOM_ID: config-starship-toml-go
    :END:
...
*** https://starship.rs/config/#google-cloud-gcloud-prod-warning
    :PROPERTIES:
    :CUSTOM_ID: google-cloud-gcloud-prod-warning
    :END:
...
*** [[https://gitlab.com/lanastara_foss/starship-jj][Jujutsu]]
    :PROPERTIES:
    :CUSTOM_ID: config-starship-toml-jujutsu
    :END:
...
**** =~/.config/starship-jj/starship-jj.toml=
     :PROPERTIES:
     :CUSTOM_ID: config-starship-jj-starship-jj-toml
...
*** Nushell
    :PROPERTIES:
    :CUSTOM_ID: config-starship-toml-nushell
    :END:
...
*** https://starship.rs/config/#terraform
    :PROPERTIES:
    :CUSTOM_ID: config-starship-toml-terraform
    :END:
...
** Smart Autocomplete & Shell History
   :PROPERTIES:
   :CUSTOM_ID: smart-autocomplete-shell-history
...
** Configuration
   :PROPERTIES:
   :CUSTOM_ID: television-configuration
...
*** Files With TODO, HACK, etc.
    :PROPERTIES:
    :CUSTOM_ID: files-with-todo-hack-etc
    ...
    #+begin_src toml
      ...
      [actions.edit]
      description = "Open file in Emacs at the comment"
      command = "emacsclient -n '+{strip_ansi|split:\\::1}' '{strip_ansi|split:\\::0}'"
      mode = "execute"
    #+end_src

*** Jujutsu Bookmark
    :PROPERTIES:
    :CUSTOM_ID: jujutsu-bookmark
