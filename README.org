...
* [[https://github.com/eza-community/eza][eza]]
  ...
  #+begin_src sh
    if command -v eza 1>/dev/null 2>&1; then
        ...
        alias ll="EZA_ICON_SPACING=1 eza --classify --icons=always --git --git-repos --long --header --smart-group"
        alias ltl="EZA_ICON_SPACING=1 eza --classify --icons=always --git --git-repos --tree --long --header --smart-group"
    fi
  #+end_src
