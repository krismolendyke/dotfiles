...
* OpenRouter
  ...
  The actual exports live with my other [[*Secrets][secrets]] in =~/.bashrc.d/secret/variables.sh= (that is where the secret
  =OPENROUTER_API_KEY= — grab one at https://openrouter.ai/keys — is kept and where these variables are really set).
  This section only /documents/ them, so the block below is illustrative and intentionally /not/ tangled; nothing here is
  exported a second time at runtime.
