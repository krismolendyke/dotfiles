...
* Secrets
  ...
  #+begin_src sh
    ...
    [ -n "$(find "${K20E_SECRET_HOME}" -mindepth 1 -maxdepth 1 2>/dev/null)" ] && chmod -Rf 0600 "${K20E_SECRET_HOME}"/*
  #+end_src
...
* PostgreSQL
  ...
  #+begin_src sh
    [ -d "${HOMEBREW_PREFIX}/opt/postgresql@16" ] && pathmunge "${HOMEBREW_PREFIX}/opt/postgresql@16/bin"
  #+end_src
  ...
  :CUSTOM_ID: skills
   ...
   #+begin_src bash
     ...
     k20e_agent_skills_sync() {
        ...
        mkdir -p "$cache_dir"
        local url name clone_dir trunk_info skill_file skill_dir skill_name agent_dir
        for url in "${K20E_AGENT_SKILL_REPOS[@]}"; do
            ...
            trunk_info="$(jj -R "$clone_dir" log -r 'trunk()' --no-graph -T 'change_id.shortest() ++ ": " ++ description.first_line()' 2>/dev/null)"
